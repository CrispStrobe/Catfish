...
def get_platform_info() -> dict:
    ...
    if system == "windows":
        ...
        return {
            ...
            "pause_cmd": 'read -p "Press any key to continue..."',
            "echo_cmd": "echo",
        }
...
def open_file_or_folder(path: Path, open_folder: bool = False):
    ...
    if not ((is_windows_path_type and is_on_windows_os) or (not is_windows_path_type and not is_on_windows_os)):
    ...
    try:
        ...
        raise FileOperationError(f"Could not open path {target}: {e}") from e
