...
from core.data_structures import ScanConfig, SearchCriteria, SearchResult
...
from utils.platform_utils import FileOperationError, calculate_window_geometry, get_screen_geometry, open_file_or_folder
...
class UniversalSearchApp:
    ...
    def get_active_indices_only(self) -> list[Path]:
        ...
        return [caf_path for caf_path in self.available_indices if self.config.is_index_active(str(caf_path))]
    ...
    def setup_search_tab(self):
        ...
        ttk.Button(search_btn_frame, text=t.get("clear_button"), command=self.clear_search_criteria).pack(
        ...
        ttk.Button(action_frame, text=t.get("open_folder"), command=self.open_search_folder).pack(
            side=tk.LEFT, padx=(10, 0)
        )
        ttk.Button(action_frame, text=t.get("copy_path"), command=self.copy_search_path).pack(
            side=tk.LEFT, padx=(10, 0)
        )
        ttk.Button(action_frame, text=t.get("export_results"), command=self.export_search_results).pack(
            side=tk.LEFT, padx=(10, 0)
        )
    ...
    def setup_manage_tab(self):
        ...
        ttk.Button(action_frame, text=t.get("refresh_indices"), command=self.refresh_indices).pack(
            side=tk.LEFT, padx=(10, 0)
        )
        ttk.Button(action_frame, text="Browse Contents", command=self.browse_index_contents).pack(
            side=tk.LEFT, padx=(10, 0)
        )
        ttk.Button(action_frame, text=t.get("delete_index"), command=self.delete_selected_index).pack(
            side=tk.LEFT, padx=(10, 0)
        )
        ...
        ttk.Button(toggle_frame, text="Deactivate All", command=self.deactivate_all_indices).pack(
            side=tk.LEFT, padx=(10, 0)
        )
    ...
    def setup_duplicates_tab(self):
        ...
        # Title
        ttk.Label(main_frame, text=t.get("duplicates_tab"), font=("TkDefaultFont", 16, "bold")).pack(pady=(0, 20))
        ...
        ttk.Button(source_frame, text=t.get("browse_button"), command=self.select_duplicate_source).pack(
        ...
        ttk.Button(dest_buttons, text=t.get("add_folder"), command=self.add_dup_dest_folder_enhanced).pack(side=tk.LEFT)
        ttk.Button(dest_buttons, text=t.get("remove_selected"), command=self.remove_dup_dest_folder_enhanced).pack(
            side=tk.LEFT, padx=(10, 0)
        )
        ttk.Button(dest_buttons, text=t.get("clear_all"), command=self.clear_dup_dest_folders_enhanced).pack(
            side=tk.LEFT, padx=(10, 0)
        )
        ...
        hash_check = ttk.Checkbutton(
            hash_frame, text=t.get("use_hash"), variable=self.dup_use_hash_var, command=self.on_dup_hash_toggle
        )
        hash_check.pack(side=tk.LEFT)

        self.dup_hash_algo_var = tk.StringVar(value=self.config.get("default_hash_algo", "md5"))
        self.dup_hash_combo = ttk.Combobox(
            hash_frame,
            textvariable=self.dup_hash_algo_var,
        ...
        ttk.Checkbutton(index_frame, text=t.get("reuse_indices"), variable=self.dup_reuse_indices_var).pack(
            side=tk.LEFT
        )
        ...
        ttk.Button(button_frame, text=t.get("start_scan"), command=self.start_duplicate_scan).pack(side=tk.LEFT)
        ttk.Button(button_frame, text=t.get("clear_button"), command=self.clear_duplicate_form_enhanced).pack(
            side=tk.LEFT, padx=(10, 0)
        )
    ...
    def setup_settings_tab(self):
        ...
        lang_combo = ttk.Combobox(
            lang_frame, textvariable=self.language_var, values=["en", "de"], width=10, state="readonly"
        )
        ...
        hash_combo = ttk.Combobox(
            hash_frame, textvariable=self.hash_var, values=["md5", "sha1", "sha256"], width=10, state="readonly"
        )
        hash_combo.pack(side=tk.LEFT)

        # Auto-load indices
        self.auto_load_var = tk.BooleanVar(value=self.config.get("auto_load_indices", True))
        ttk.Checkbutton(main_frame, text=t.get("auto_load_indices"), variable=self.auto_load_var).pack(
        ...
        scrollbar_settings = ttk.Scrollbar(
            list_frame_settings, orient=tk.VERTICAL, command=self.locations_listbox.yview
        )
        ...
        ttk.Button(locations_buttons, text=t.get("add_location"), command=self.add_search_location).pack(side=tk.LEFT)
        ttk.Button(locations_buttons, text=t.get("remove_location"), command=self.remove_search_location).pack(
            side=tk.LEFT, padx=(10, 0)
        )

        # Apply settings button
        ttk.Button(main_frame, text=t.get("apply_settings"), command=self.apply_settings).pack(pady=(20, 0))
    ...
    def add_dup_dest_folder_enhanced(self):
        ...
        if folder:
            ...
            if related_indices:
                ...
                self.dup_dest_tree.insert(
                    "",
                    "end",
                    text=str(folder_path),
                    values=(index_info["path"].name, last_updated, "☐"),
    ...
    def find_indices_for_folder(self, folder_path: Path) -> list[dict]:
        ...
        for caf_path in active_indices:
            ...
            if info:
                ...
                try:
                    if folder_path.resolve().is_relative_to(Path(info.root_path).resolve()) or Path(
                        info.root_path
                    ).resolve().is_relative_to(folder_path.resolve()):
    ...
    def show_index_selection_dialog(self, folder_path: Path, related_indices: list[dict]) -> Optional[dict]:
        ...
        ttk.Label(
            dialog, text=f"{t.get('select_indices_to_update')}\n{folder_path}", font=("TkDefaultFont", 10, "bold")
        ).pack(pady=10)
    ...
    def parse_search_criteria(self) -> SearchCriteria:
        ...
        try:
            ...
            raise ValueError(t.get("invalid_size", e)) from e
        ...
        try:
            ...
            raise ValueError(t.get("invalid_date", e)) from e

        return SearchCriteria(
            name_pattern=name_pattern, size_min=size_min, size_max=size_max, date_min=date_min, date_max=date_max
        )
            ...
            try:
                with open(filename, "w", encoding="utf-8") as f:
                    ...
                    for i, result in enumerate(self.search_results):
                        ...
                        f.write(
                            f'"{filename_clean}","{size_str}",{result.size},"{modified_str}","{index_name}","{path_clean}"\n'
                        )
    ...
    def start_duplicate_scan(self):
        ...
        if invalid_paths:
            messagebox.showerror(
                t.get("error"), "Invalid destination folders:\n" + "\n".join(str(p) for p in invalid_paths)
            )
            return
        ...
        config = ScanConfig(
            ...
            recreate_indices=len(indices_to_recreate) > 0,
        )
    ...
    def apply_settings(self):
        ...
        if old_lang != new_lang:
            messagebox.showinfo("Language Changed", "Please restart the application to apply language changes.")
        else:
        ...
        def search_thread():
            try:
                ...
                for i, caf_path in enumerate(active_indices):
                    ...
                    def specific_search_progress(current, total, speed, _name=index_name):
                        pct = int((current / total) * 100) if total > 0 else 0
                        msg = f"Scanning {current:,}/{total:,} ({pct}%)"
                        if speed > 0:
                            msg += f" @ {int(speed):,} items/s"
                        gui_msg_callback(f"Searching {_name}", msg)

                    # 3. Call CORE Search Logic
                    results = search_files_in_index(file_index, criteria, progress_callback=specific_search_progress)
