...
class DuplicateResultsWindow:
    ...
    def setup_ui(self):
        ...
        ttk.Radiobutton(
            mode_frame,
            text=t.get("destination_duplicates"),
            variable=self.selection_mode_var,
            value="destination",
            command=self.on_selection_mode_change,
        ...
        ttk.Button(filter_inner, text=t.get("select_all_filtered"), command=self.select_all_filtered).pack(
        ...
        ttk.Button(action_frame, text=t.get("generate_script"), command=self.generate_script).pack(
    ...
    def on_tree_click(self, event):
        ...
        if item:
            ...
            if (mode == "source" and "source" in tags and "hidden" not in tags) or (
                mode == "destination" and "destination" in tags and "hidden" not in tags
            ):
    ...
    def update_status(self):
        ...
        if count > 0:
            try:
                total_size = sum(
                    Path(self.tree.item(item, "values")[1]).stat().st_size for item in self.selected_for_deletion
                )
    ...
    def generate_script(self):
        ...
        file_types = [(f"{platform_info['name']} scripts", f"*{platform_info['script_ext']}"), ("All files", "*.*")]
