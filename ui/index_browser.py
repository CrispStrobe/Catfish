...
from utils.file_utils import format_size, path_is_native_and_exists
...
class IndexBrowserWindow:
    ...
    def load_index_contents(self):
        ...
        try:
            ...
            self.file_entries = []
            for _size, entries in file_index.size_index.items():
    ...
    def populate_files_tree(self, filter_text=None):
        ...
        if filter_text:
            try:
                ...
                entries_to_show = [
                    entry
                    for entry in self.file_entries
                    if filter_text.lower() in entry.path.name.lower() or filter_text.lower() in str(entry.path).lower()
                ]
        ...
        for entry in entries_to_show:
            ...
            self.files_tree.insert(
                "", "end", text=filename, values=(size_str, modified_str, display_path, exists_str), tags=tags
            )
    ...
    def export_file_list(self):
        ...
        filename = filedialog.asksaveasfilename(
            title="Export File List", defaultextension=".csv", filetypes=[("CSV files", "*.csv"), ("All files", "*.*")]
        )
            ...
            try:
                with open(filename, "w", encoding="utf-8") as f:
                    ...
                    for item in self.files_tree.get_children():
                        ...
                        size_bytes = next(
                            (entry.size for entry in self.file_entries if str(entry.path) == values[2]), 0
                        )
